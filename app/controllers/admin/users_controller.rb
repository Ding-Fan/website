  ...
  class UsersController < Admin::ApplicationController
    def index
      ...
      if params[:q].present?
        ...
        @users = @users.where("lower(username) LIKE ? or lower(email) LIKE ?", qstr, qstr)
      end
    ...
    def clean
      ...
      if params[:type] == "replies"
        ...
        redirect_to edit_admin_user_path(@user.id), notice: "最近 10 条删除，成功 #{@user.username} 还有 #{count} 条回帖。"
      end
    end
  end
end
