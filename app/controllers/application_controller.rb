...
class ApplicationController < ActionController::Base
  ...
  def render_optional_error_file(status_code)
    ...
    fname = %w(403 404 422 500).include?(status) ? status : "unknown"

    respond_to do |format|
      format.html { render template: "/errors/#{fname}", handler: [:erb], status: status, layout: "error" }
  ...
  def user_locale
    params[:locale] || cookies[:locale] || http_head_locale || Figaro.env.default_locale || I18n.default_locale
  end
