...
module UsersHelper
  ...
  # 生成用户 user 的链接，user 参数可接受 user 对象或者 字符串的 username
  def user_name_tag(user, options = {})
    ...
    link_to(user, main_app.user_path(user), options)
  end
  ...
  def render_user_level_tag(user)
    ...
    level_class = case user.level
                  ...
                  when "normal"  then "label-success"
                  when "banned"  then "label-warning"
  ...
  def block_user_tag(user)
    ...
    icon = '<i class="fa fa-eye-slash"></i>'
    if blocked
      link_to raw("#{icon} <span>取消屏蔽</span>"), "#", title: "忽略后，社区首页列表将不会显示此用户发布的内容。", "data-id" => user.username, class: "#{class_names} active"
    else
      link_to raw("#{icon} <span>屏蔽</span>"), "#", title: "", "data-id" => user.username, class: class_names
    end
  end
  ...
  def follow_user_tag(user, opts = {})
    ...
    if followed
      link_to raw("#{icon} <span>取消关注</span>"), "#", "data-id" => username, class: "#{class_names} active"
    else
      link_to raw("#{icon} <span>关注</span>"), "#", title: "", "data-id" => username, class: class_names
    end
  end
end
