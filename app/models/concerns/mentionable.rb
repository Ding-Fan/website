module Mentionable
  ...
  def mentioned_user_usernames
    # 用于作为缓存 key
    ids_md5 = Digest::MD5.hexdigest(mentioned_user_ids.to_s)
    Rails.cache.fetch("#{self.class.name.downcase}:#{id}:mentioned_user_usernames:#{ids_md5}") do
      self.mentioned_users.pluck(:username)
    end
  end

  def extract_mentioned_users
    logins = body.scan(/@([#{User::username_FORMAT}]{3,20})/).flatten.map(&:downcase)
    if logins.any?
