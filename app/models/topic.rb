...
class Topic < ApplicationRecord
  ...
  def tags(tag, page = 20)
    self.tagged_with(tag).by_join_date.paginate(:page => params[:page], :per_page => page)
  end
  ...
  def update_last_reply(reply, opts = {})
    ...
    self.last_reply_user_username = reply.try(:user_username)
  ...
  def destroy_by(user)
    ...
    update_attribute(:who_deleted, user.username)
    destroy
  end
