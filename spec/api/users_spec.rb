...
describe 'API V3', 'users', type: :request do
  describe 'GET /api/v3/users.json' do
    ...
    it 'should work' do
      ...
      expect(json['users'][0]).to include(*%w(id username avatar_url))
    end
  ...
  describe 'GET /api/v3/users/:username.json' do
    it 'should get user details with list of topics' do
      user = create(:user, username: 'test user', email: 'foobar@gmail.com', email_public: true)
      get '/api/v3/users/test_user.json'
      expect(response.status).to eq 200
      fields = %w(id username email avatar_url github google facebook twitter website bio tagline
    ...
    it 'should not hidden email when current_user itself' do
      login_user!
      get "/api/v3/users/#{current_user.username}.json"
      expect(response.status).to eq 200
      expect(json['user']['email']).to eq current_user.email
    end
  end

  describe 'GET /api/v3/users/:username/topics.json' do
    let(:user) { create(:user) }

    describe 'recent order' do
      it 'should work' do
        @topics = create_list(:topic, 3, user: user)
        get "/api/v3/users/#{user.username}/topics.json", offset: 0, limit: 2
        expect(response.status).to eq 200
        expect(json['topics'].size).to eq 2
        fields = %w(id title user node_name node_id last_reply_user_id
                    last_reply_user_username created_at updated_at replies_count)
    ...
    describe 'hot order' do
      it 'should work' do
        ...
        get "/api/v3/users/#{user.username}/topics.json", order: 'likes', offset: 0, limit: 3
        expect(response.status).to eq 200
    ...
    describe 'hot order' do
      it 'should work' do
        ...
        get "/api/v3/users/#{user.username}/topics.json", order: 'replies', offset: 0, limit: 3
        expect(response.status).to eq 200
        expect(json['topics'].size).to eq 3
        expect(json['topics'][0]['id']).to eq @hot_topic.id
      end
  ...
  describe 'GET /api/v3/users/:username/replies.json' do
    let(:user) { create(:user) }
    ...
    describe 'recent order' do
      it 'should work' do
        ...
        get "/api/v3/users/#{user.username}/replies.json", offset: 0, limit: 2
  ...
  describe 'GET /api/v3/users/:username/favorites.json' do
    let(:user) { create(:user) }
    ...
    it 'should work' do
      ...
      get "/api/v3/users/#{user.username}/favorites.json", offset: 1, limit: 2
      expect(response.status).to eq 200
      expect(json['topics'].size).to eq 2
      fields = %w(id title user node_name node_id last_reply_user_id
                  last_reply_user_username created_at updated_at replies_count)
  ...
  describe 'GET /api/v3/users/:username/followers.json' do
    let(:user) { create(:user) }
    ...
    it 'should work' do
      ...
      get "/api/v3/users/#{user.username}/followers.json", offset: 0, limit: 2
      expect(response.status).to eq 200
  ...
  describe 'GET /api/v3/users/:username/blocked.json' do
    let(:user) { create(:user) }

    it 'require login' do
      get "/api/v3/users/#{user.username}/blocked.json"
      expect(response.status).to eq 401
    end
    ...
    it 'should work' do
      ...
      get "/api/v3/users/#{current_user.username}/blocked.json", offset: 0, limit: 2
      expect(response.status).to eq 200
  ...
  describe 'GET /api/v3/users/:username/following.json' do
    let(:user) { create(:user) }
    ...
    it 'should work' do
      ...
      get "/api/v3/users/#{user.username}/following.json", offset: 0, limit: 2
      expect(response.status).to eq 200
  ...
  describe 'POST /api/v3/users/:username/follow.json / unfollow' do
    let(:user) { create(:user) }

    it 'should 401 when nologin' do
      post "/api/v3/users/#{user.username}/follow.json"
      expect(response.status).to eq 401

      post "/api/v3/users/#{user.username}/unfollow.json"
      expect(response.status).to eq 401
    end
  ...
  describe 'POST /api/v3/users/:username/block.json / unblock.json' do
    let(:user) { create(:user) }

    it 'should 401 when nologin' do
      post "/api/v3/users/#{user.username}/block.json"
      expect(response.status).to eq 401

      post "/api/v3/users/#{user.username}/unblock.json"
      expect(response.status).to eq 401
    end
