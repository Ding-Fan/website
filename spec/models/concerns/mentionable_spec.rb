...
describe Mentionable, type: :model do
  it 'should work with chars' do
    ...
    doc = TestDocument.create body: "@#{user.username} @#{user1.login}", user: create(:user)
    expect(doc.mentioned_user_usernames).to include(user.username, user1.login)

    doc = TestDocument.create body: "@#{user.username.upcase} @#{user1.login.downcase}", user: create(:user)
    expect(doc.mentioned_user_usernames).to include(user.username, user1.login)
  end

  it 'should extract mentioned user ids' do
    user = create :user
    doc = TestDocument.create body: "@#{user.username}", user: create(:user)
    expect(doc.mentioned_user_ids).to eq([user.id])
    expect(doc.mentioned_user_usernames).to eq([user.username])
  end
  ...
  it 'except self user' do
    user = create :user
    doc = TestDocument.create body: "@#{user.username}", user: user
  ...
  it 'should get mentioned user logins' do
    ...
    expect(doc.mentioned_user_usernames).to match_array([user1.login, user2.login])
  end

  it 'should send mention notification' do
    user = create :user
    expect do
      TestDocument.create body: "@#{user.username}", user: create(:user)
    end.to change(user.notifications.unread, :count)

    expect do
      TestDocument.create(body: "@#{user.username}", user: user)
    end.not_to change(user.notifications.unread, :count)

    expect do
      TestDocument.create(body: "@#{user.username}", user: create(:user)).destroy
    end.not_to change(user.notifications.unread, :count)
  end

  it 'should send mention to reply_to user' do
    user = create :user
    last_doc = TestDocument.create body: "@#{user.username}", user: user
    user1 = create :user
    expect do
